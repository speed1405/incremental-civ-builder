...
export interface BuildingType {
  ...
  maxCount: number; // Maximum number of this building
}
...
export const BUILDING_TYPES: BuildingType[] = [
  // Stone Age Buildings
  {
    ...
    cost: { food: 20, wood: 30, stone: 10, gold: 0 },
    buildTime: 10,
    production: { food: 0.5 },
    maxCount: 10,
  },
  {
    ...
    era: 'stone_age',
    unlockTech: 'fire',
    cost: { food: 10, wood: 50, stone: 20, gold: 0 },
    buildTime: 8,
  ...
  },
  {
    ...
    cost: { food: 30, wood: 40, stone: 10, gold: 0 },
    buildTime: 15,
    production: { food: 1.0 },
    maxCount: 15,
  },
...
export function getBuildingsByEra(era: string): BuildingType[] {
...
export function canBuildBuilding(
  ...
  if (buildingType.unlockTech && !unlockedBuildings.has(buildingType.id)) return false;
  
  // Check if we've reached max count
  const existing = builtBuildings.find(b => b.typeId === buildingId);
  if (existing && existing.count >= buildingType.maxCount) return false;
  
  // Check costs
  if (resources.food < buildingType.cost.food) return false;
  if (resources.wood < buildingType.cost.wood) return false;
  if (resources.stone < buildingType.cost.stone) return false;
  if (resources.gold < buildingType.cost.gold) return false;
  
  return true;
}

export function calculateBuildingProduction(buildings: Building[]): {
  food: number;
  wood: number;
  stone: number;
  gold: number;
...
export function getTotalBuildingCount(buildings: Building[]): number {
