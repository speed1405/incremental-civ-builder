...
export function getMissionsByEra(missions, era) {
    return missions.filter(m => m.era === era);
}
export function getMissionById(missions, id) {
...
function getBattleMessage(round, playerDamage, enemyDamage, isPlayerTurn) {
...
export function simulateBattleRound(playerHealth, enemyHealth, playerAttack, playerDefense, enemyAttack, enemyDefense, round) {
...
export function generateBattleLogs(playerArmy, enemyArmy) {
    const logs = [];
...
export function determineBattleResult(logs, playerStartHealth, enemyStartHealth, mission) {
...
export function simulateCombat(playerArmy, mission) {
...
export function canStartMission(playerArmy) {
...
export function isMissionAvailable(mission, currentEra) {
...
export function getTerritoriesByEra(territories, era) {
    return territories.filter(t => t.era === era);
}
export function isTerritoryAvailable(territory, currentEra) {
    const territoryEraIndex = getEraIndex(territory.era);
    const currentEraIndex = getEraIndex(currentEra);
    return territoryEraIndex <= currentEraIndex;
}
export function calculateConquestBonuses(territories) {
...
export function calculateConquestMultipliers(territories) {
...
export function simulateTerritoryConquest(playerArmy, territory) {
    const playerPower = calculateArmyPower(playerArmy);
    // If player has no army, they lose instantly
    if (playerPower.health <= 0) {
        return {
